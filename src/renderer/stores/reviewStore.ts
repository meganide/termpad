  ...
  create<ReviewStore>((set, get) => ({
    ...
    openReview: async (projectPath, baseBranch, compareBranch) => {
      ...
      try {
        ...
        const currentBranch =
          compareBranch || (await window.terminal.getCurrentBranch(projectPath));
    ...
    openWorkingTreeReview: async (projectPath, files, headCommit, baseBranch = 'HEAD') => {
      set({ isLoading: true, error: null, projectPath });
      ...
      try {
              ...
              reviewData = {
                ...
                files:
                  newFiles.length > 0
                    ? [
                        ...reviewData.files,
                        ...newFiles.map((f) => ({ path: f.path, viewed: false })),
                      ]
                    : reviewData.files,
                lastCommitHash: headCommit,
                updatedAt: new Date().toISOString(),
        ...
        const message =
          error instanceof Error ? error.message : 'Failed to open working tree review';
        set({ error: message, isLoading: false });
      }
    },
...
export const ReviewStoreContext = createContext(defaultReviewStore);
const identity = (state: ReviewStore) => state;

// Existing standalone windows keep the default store; each embedded review
// provides its own store so comments and selection cannot cross worktrees.
export const useReviewStore = Object.assign(function useReviewStore<T = ReviewStore>(
  selector: (state: ReviewStore) => T = identity as (state: ReviewStore) => T
): T {
  return useStore(useContext(ReviewStoreContext), selector);
}, defaultReviewStore);
