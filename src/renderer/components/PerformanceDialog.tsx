...
export function PerformanceDialog({
  ...
  return (
    <Dialog
      open
      onOpenChange={(open) => {
        if (!open && !stopping) onClose();
      }}
    >
      <DialogContent
        className="flex h-[min(800px,90vh)] flex-col gap-0 overflow-hidden p-0 sm:max-w-6xl"
      ...
      >
        <DialogHeader className="shrink-0 border-b border-border p-6">
          ...
          <DialogDescription>
            Everything running in Termpad, across all repositories.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2 px-6 pb-4">
          ...
          <div
            ...
            role="group"
            aria-label="Filter running items"
          >
            {(['all', 'terminal', 'process', 'browser', 'app'] as const).map((kind) => (
              <Button
                key={kind}
                size="sm"
        ...
        {selected && (
          ...
          >
            ...
            <Button variant="destructive" size="sm" disabled={stopping} onClick={() => void stop()}>
              {stopping
                ? 'Closing…'
                : selected.kind === 'process'
                  ? 'Stop process'
              ...
              {rows.map((row) => {
                ...
                return (
                  <tr key={row.id} className="border-b border-border/40 hover:bg-muted/40">
                    ...
                    <td className="px-3 py-3 text-xs text-muted-foreground">{kinds[row.kind]}</td>
                      ...
                      <div className="flex justify-end gap-1">
                        {canOpen && (
                          <Tooltip>
                            ...
                            <TooltipContent>
                              {row.kind === 'process' ? 'Open owning terminal' : 'Go to item'}
                            </TooltipContent>
                          </Tooltip>
                        )}
                        {canClose && (
        ...
        <div className="flex shrink-0 flex-wrap items-center justify-between gap-2 px-6 py-4 text-xs text-muted-foreground">
          <p className="max-w-2xl">
            Totals count each process once. Terminal rows include their children; browser usage may
            be shared. CPU: 100% = one core. Totals use available readings. — = waiting for a
            reading.
          </p>
